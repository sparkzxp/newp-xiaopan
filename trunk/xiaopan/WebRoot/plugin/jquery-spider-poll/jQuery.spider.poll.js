(function ($){
	var defaults = {
		...
		bgImg:'plugin/jquery-spider-poll/images/bg.gif',//背景
		multiple:false,//是否多选
		...
		$.each(dataObj.root,function (i,n){
			...
			if(options.showPoll){//是否显示投票
				if(options.multiple){//是否是多选
					$("#"+id+" tr:last").append("<td><input type='checkbox' name='poll_"+id+"' value='"+n.id+"'/></td>");
				}else {
					$("#"+id+" tr:last").append("<td><input type='radio' name='poll_"+id+"' value='"+n.id+"'/></td>");
				}
			}
		});
