		...
		.extend(
				...
				$.fn.datagrid.defaults.view,
				{
					addExpandColumn : function(target, index) {
						...
						function _add(rowIndex) {
							var tr = opts.finder.getTr(target, rowIndex,
									'body', 1);
							if (tr.find('span.datagrid-row-expander').length) {
								return;
							} // the expander is already exists
							var cc = [];
							cc.push('<td>');
							cc
									.push('<div style="text-align:center;width:25px;height:16px;">');
							cc
									.push('<span class="datagrid-row-expander datagrid-row-expand" style="display:inline-block;width:16px;height:16px;cursor:pointer;" />');
							cc.push('</div>');
							cc.push('</td>');
							if (tr.is(':empty')) {
								tr.html(cc.join(''));
								...
								$(cc.join(''))
										.insertAfter(
												tr
														.children('td.datagrid-td-rownumber'));
							} else {
								$(cc.join('')).insertBefore(
										tr.children('td:first'));
							}
							$(target).datagrid('fixRowHeight', rowIndex);
							tr.find('span.datagrid-row-expander').unbind(
									'.datagrid').bind(
									'click.datagrid',
									function(e) {
										...
										if ($(this).hasClass(
												'datagrid-row-expand')) {
											$(target).datagrid('expandRow',
													rowIndex);
										} else {
											$(target).datagrid('collapseRow',
													rowIndex);
										}
					...
					render : function(target, container, frozen) {
						...
						for ( var i = 0; i < rows.length; i++) {
							...
							var cls = (i % 2 && opts.striped) ? 'class="datagrid-row datagrid-row-alt"'
									: 'class="datagrid-row"';
							var styleValue = opts.rowStyler ? opts.rowStyler
									.call(target, i, rows[i]) : '';
							...
							table.push('<tr id="' + rowId
									...
									+ ' ' + style + '>');
							table.push(this.renderRow.call(this, target,
									fields, frozen, i, rows[i]));
							table.push('</tr>');
							...
							if (frozen) {
								...
								table.push(opts.detailFormatter.call(target, i,
										rows[i]));
							}
							table.push('</div>');
							table.push('</td>');
					...
					insertRow : function(target, index, row) {
						...
						$.fn.datagrid.defaults.view.insertRow.call(this,
								target, index, row);
						...
						function _insert(frozen) {
							...
							newDetail.hide();
							if (!frozen) {
								newDetail.find('div.datagrid-row-detail').html(
										opts.detailFormatter.call(target,
												index, row));
							}
						}
					},
					...
					deleteRow : function(target, index) {
						...
						$.fn.datagrid.defaults.view.deleteRow.call(this,
								target, index);
					...
					updateRow : function(target, rowIndex, row) {
						var dc = $.data(target, 'datagrid').dc;
						var opts = $.data(target, 'datagrid').options;
						var cls = $(target).datagrid('getExpander', rowIndex)
								.attr('class');
						$.fn.datagrid.defaults.view.updateRow.call(this,
								target, rowIndex, row);
						this.addExpandColumn.call(this, target, rowIndex);
						$(target).datagrid('getExpander', rowIndex).attr(
								'class', cls);

						// update the detail content
						if (this.canUpdateDetail) {
							var row = $(target).datagrid('getRows')[rowIndex];
							var detail = $(target).datagrid('getRowDetail',
									rowIndex);
							detail.html(opts.detailFormatter.call(target,
									rowIndex, row));
						}
					},
					...
					onBeforeRender : function(target) {
						...
						var t = dc.view1.children('div.datagrid-header').find(
								'table');
						...
						var td = $('<td rowspan="'
								...
								+ '"><div class="datagrid-header-expander" style="width:25px;"></div></td>');
						if ($('tr', t).length == 0) {
							td.wrap('<tr></tr>').parent().appendTo(
									$('tbody', t));
					...
					onAfterRender : function(target) {
						...
						function setBodyTableWidth() {
							var columnWidths = dc.view2.children(
									'div.datagrid-header').find('table')
									.width();
						...
						this.canUpdateDetail = true; // define if to update
														// the detail content
														// when 'updateRow'
														// method is called;

						dc.footer1.find('span.datagrid-row-expander').css(
								'visibility', 'hidden');
...
$.extend($.fn.datagrid.methods, {
	...
	getExpander : function(jq, index) { // get row expander object
		...
		return opts.finder.getTr(jq[0], index, 'body', 1).find(
	...
	expandRow : function(jq, index) {
		return jq.each(function() {
			...
			if (expander.hasClass('datagrid-row-expand')) {
				expander.removeClass('datagrid-row-expand').addClass(
	...
	collapseRow : function(jq, index) {
		return jq.each(function() {
			...
			if (expander.hasClass('datagrid-row-collapse')) {
				expander.removeClass('datagrid-row-collapse').addClass(
						'datagrid-row-expand');
				var tr1 = opts.finder.getTr(this, index, 'body', 1).next();
				var tr2 = opts.finder.getTr(this, index, 'body', 2).next();
				tr1.hide();
